...
import { FaTaxi, FaPlane, FaSuitcase, FaUsers, FaClock, FaRoute } from 'react-icons/fa';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {services.map((service, index) => (
              <motion.div
                ...
                className="glass-effect rounded-xl p-8 group hover:border-primary border border-gray-700 transition-all duration-300"
              >
                <div className="mb-6 transform group-hover:scale-110 transition-transform duration-300">
                  {service.icon}
                </div>
                <h3 className="text-2xl font-semibold text-white mb-4">{service.title}</h3>
                <p className="text-lightGray mb-6">{service.description}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {features.map((feature, index) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="glass-effect rounded-xl p-6 flex items-center gap-6"
              >
